...
export const Navbar: React.FC = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md w-full transition-all duration-300 ${
          ...
          {links.map((link) => (
            <React.Fragment key={link.path}>
              {link.children ? (
                ...
                <Link
                  href={link.path}
                  className={`text-lg py-2 border-b border-border transition-colors ${
                    pathname === link.path
                      ? "font-medium text-primary"
                      : "text-foreground hover:text-primary"
                  }`}
